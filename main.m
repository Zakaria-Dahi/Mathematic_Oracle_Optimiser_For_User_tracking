...
% Authors: Zakaria Abdelmoiz Dahi (1), Enrique Alba (2) and Gabriel Luque (3)
% Affiliation: (1) Depatment of Fundamental Computer Science and Its Applications, University of Constantine 2, Algeria.
% Affiliation: (2 and 3) Department de Lenguajes y Ciencias de la Computacion, University of Malaga, Spain.
...
%% Prameters  of the TD-EA  ----------------------
     pcr = 1;  
     pm = 0.5 - ((0.5 - (1/Dimension))/2); 
     lambda_omega = 1.5 - ((1.5 - 1.1)/2); 
     gamma = 0.9 - ((0.9 - 0.5)/2);
%% Parameters of the mathematical Oracle ---------------------
     Alpha = 1.02;
     Period = 30; 
     decrease_step_gamma = ((0.9 - 0.5)/2)/ (501/Period);
     increase_step_gamma = ((0.9 - 0.5)/2)/ (501/Period); 
     decrease_step_lambda = ((1.5 - 1.1)/2)/ (501/Period); 
     increase_step_lambda = ((1.5 - 1.1)/2)/ (501/Period); 
     if Dimension <= 36 
	    lower_threshold = 60; 
     else
	    lower_threshold = 50; 
     ...
     upper_threshold = 70; 
%% Paremetrs of the local search n° 1 -----------------------------
   global LS_period
   if ind == 11
...
%% Paremetrs of the local search n° 2 -----------------------------
   global D
...
%% variable to save for each execution  ----------------------
     %%%%   Fitness of the best individual reached after the end of all the ieteration 
     ...
     global ecart 
            ecart = 0;
     %%%% to save all the information related to the experiment 
     global vect_one 
            vect_one = [];
     global vect_two
            vect_two = [];
     ...
     %%%% to save the best individual all along the "Nexe" execution 
     global GBEST_ALL
            GBEST_ALL = [];
...
%% variable used during the calculation ---------------------------------------
     %%% Vector containing the fitness of all individuals of the population 
            fit = [];
     ...
     %%%% The population 
            population = [];
     %%% To save the iteration and the corresponding fitness value of this iteration (fitness evauation)
            ALL_FITNESSES = [];
            ALL_ITTERATIONS = [];
...
% ------------------------------------------------------------------------
%% -- calculate the best individual (its index, fitness, etc.) ---
...
it = it + (2*individual);
%% ---------- Test if the period to Apply the mathematical oracle is reached or no --------------------------
if count_down ~= 0
...
% -- if the threshold is equal to zero, we engage the cataclysmic restarting --
if HUX_distance == 0
    reste = iter - it; 
    if reste > 0
        if reste >= indiv 
